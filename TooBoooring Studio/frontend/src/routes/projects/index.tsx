...
function ProjectsIndex() {
  ...
  const [useProjectsFolder, setUseProjectsFolder] = useState(true) // Create TooBoooringStudioProjects subfolder
  const [importDialogOpen, setImportDialogOpen] = useState(false)
  const [importProgress, setImportProgress] = useState<ImportProgress | null>(null)
  const [importError, setImportError] = useState<string | null>(null)
  const [isImporting, setIsImporting] = useState(false)

  const PROJECTS_FOLDER_NAME = 'TooBoooringStudioProjects'

  // Extract project name from bundle filename
  // Handles both "myproject.tooboooring.zip" and browser-renamed "myproject.tooboooring (1).zip"
  const extractProjectName = (fileName: string): string => {
    ...
    // Remove .tooboooring suffix
    name = name.replace(/\.tooboooring$/i, '')
    return name
  }

  // Check if file is a valid bundle (handles browser-renamed files like "project.tooboooring (1).zip")
  const isValidBundleFile = (fileName: string): boolean => {
    // Match: anything.tooboooring.zip or anything.tooboooring (N).zip
    return /\.tooboooring(\s*\(\d+\))?\.zip$/i.test(fileName)
  }
  ...
  const handleFileSelect = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate file extension (handles browser-renamed files like "project.tooboooring (1).zip")
  ...
  const handleStartImport = async () => {
    ...
    try {
      // If useProjectsFolder is enabled, create/get the TooBoooringStudioProjects subfolder first
      ...
      if (useProjectsFolder) {
        try {
          ...
          logger.error('Failed to create TooBoooringStudioProjects folder:', err)
        ...
        <div className="panel-header border-b border-border" data-no-marquee>
          <div className="max-w-[1920px] mx-auto px-6 py-5 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Button variant="outline" size="lg" className="gap-2 px-4" asChild>
                <a
                  href="https://github.com/yourusername/tooboooring"
                  target="_blank"
          ...
          {importError && !pendingImportFile ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {/* TooBoooringStudioProjects subfolder option */}
