...
const ACCEPTED_APP_UPDATE_SIGNATURE_KEY = 'tooboooring-accepted-app-update-signature'
...
async function showSaveBeforeReloadFailedToast() {
  window.dispatchEvent(new Event('tooboooring:ensure-toaster'))
  try {
...
async function checkForAppShellUpdate() {
  ...
  try {
    const response = await fetch(`/?__tooboooring_update_check=${Date.now()}`, {
      cache: 'no-store',
      headers: {
