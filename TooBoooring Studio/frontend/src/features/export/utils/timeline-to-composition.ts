 ...
 */
function calculateSplitKeyframeOffset(item: TimelineItem, originalItems: TimelineItem[]): number {
  ...
  const siblingItems = originalItems.filter(
    (i) => i.originId === item.originId || i.id === item.originId,
  )
  ...
  const originalSourceStart = Math.min(...mediaItems.map((i) => i.sourceStart ?? 0))
