...
function renderFadeTransition(
  ...
  drawFadeParticipant(ctx, rightCanvas, canvas, getFadeScale(p, false), getFadeOpacity(p, false))

  ctx.globalCompositeOperation = 'lighter'
  drawFadeParticipant(ctx, leftCanvas, canvas, getFadeScale(p, true), getFadeOpacity(p, true))
  ctx.restore()
}
