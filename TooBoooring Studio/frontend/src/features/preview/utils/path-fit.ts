...
export function getPathBounds(vertices: MaskVertex[]): {
  ...
  for (let i = 0; i < vertices.length; i++) {
    ...
    for (const t of samples) {
      updateBounds(cubicPointAt(p0x, p1x, p2x, p3x, t), cubicPointAt(p0y, p1y, p2y, p3y, t))
    }
  }
...
export function fitShapePathToBounds(
  ...
  return {
    pathVertices: vertices.map((vertex) => ({
      ...
      inHandle: [vertex.inHandle[0] / spanX, vertex.inHandle[1] / spanY],
      outHandle: [vertex.outHandle[0] / spanX, vertex.outHandle[1] / spanY],
    })),
    transform: {
      ...existingTransform,
