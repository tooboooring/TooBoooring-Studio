...
export function usePreviewRuntimeRefs() {
  ...
  const ensureFastScrubRendererRef = useRef<() => Promise<PreviewCompositionRenderer | null>>(
    async () => null,
  )
  ...
  const transitionSessionStallCountRef = useRef<Map<string, { ct: number; count: number }>>(
    new Map(),
  )
  ...
  const captureCanvasSourceInFlightRef = useRef<Promise<
