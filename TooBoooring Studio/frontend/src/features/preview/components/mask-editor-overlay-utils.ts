...
export function cubicPointAt(p0: number, p1: number, p2: number, p3: number, t: number): number {
  const invT = 1 - t
...
export function distanceToLineSegment(
  ...
  const t = Math.max(0, Math.min(1, ((pointX - startX) * dx + (pointY - startY) * dy) / lenSq))
