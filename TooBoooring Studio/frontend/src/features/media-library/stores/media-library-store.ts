...
function getProxyCapableVideoItems(mediaItems: MediaMetadata[]): MediaMetadata[] {
  return mediaItems.filter((item) => proxyService.canGenerateProxy(item.mimeType))
}
...
const newStore: MediaLibraryStoreApi =
  hotStore ??
  create<MediaLibraryState & MediaLibraryActions>()(
    devtools(
      (set, get) => ({
        // Initial state
        ...
        clearSelection: () => {
          const state = get()
          if (state.selectedMediaIds.length === 0 && state.selectedCompositionIds.length === 0)
            return
        ...
        showNotification: (notification: MediaLibraryNotification) => {
          ...
          notificationTimeoutId = setTimeout(() => {
            ...
            set((state) => (state.notification === notification ? { notification: null } : state))
          }, 4000)
        },
