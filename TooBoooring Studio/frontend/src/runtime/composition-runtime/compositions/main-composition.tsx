...
const FrameResolvedActiveMasksProvider: React.FC<ActiveMasksProviderProps> = ({
  ...
  const activeMasks = useMemo<MaskInfo[]>(() => {
    ...
    const nextMasks = materializeMaskInfos(
      resolveActiveShapeMasksAtFrame(masks, {
...
const MaskedItem: React.FC<{
  ...
  return (
    <Item
      ...
      muted={muted}
      visible={visible}
      masks={getMasksForTrackOrder(masks, itemTrackOrder)}
...
export const MainComposition: React.FC<MainCompositionProps> = ({
  ...
  const videoAudioItems = useMemo(
    () =>
      renderPlan.videoItems.filter(
        (item) => !item.embeddedAudioMuted && !hasLinkedAudioCompanion(audioItems, item),
      ),
  ...
  const shouldUseCustomDecoder = useCallback(
    (segment: VideoAudioSegment | AudioSegment): boolean => {
      if (!segment.mediaId) {
  ...
  const visibleAdjustmentLayers =
    renderPlan.visibleAdjustmentLayers as AdjustmentLayerWithTrackOrder[]
