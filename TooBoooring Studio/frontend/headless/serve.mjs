// TooBoooringStudio headless render service.
//
...
async function main() {
  ...
  const tmpDir = path.join(os.tmpdir(), 'tooboooring-serve')
  ...
  const handleRender = async (req, res, { normalizeInline = false } = {}) => {
    ...
    res.writeHead(200, {
      ...
      ...(summary.warnings?.length
        ? { 'X-TooBoooringStudio-Warnings': warningsHeaderValue(summary.warnings) }
        : {}),
    })
  ...
  const handleEdit = async (req, res) => {
    ...
    const result = await queue.enqueue(
      () =>
        session.page.evaluate((payload) => window.tooboooring.editProject(payload), {
          project,
          ops,
          media,
        }),
  ...
  const browserNormalize = (project) =>
    queue.enqueue(
      () => session.page.evaluate((value) => window.tooboooring.normalizeProject(value), project),
    ...
    const result = await withIdempotency(
      ...
      async () => {
        const project = await queue.enqueue(
          () => session.page.evaluate((value) => window.tooboooring.createProject(value), body),
  ...
  const handleV1MediaProbe = async (req, res, id) => {
    ...
    const probe = await queue.enqueue(
      () =>
        session.page.evaluate((payload) => window.tooboooring.probeMedia(payload), {
          url: mediaUrlOf(id),
  ...
  const handleFrame = async (req, res) => {
    ...
    const summary = await queue.enqueue(
      async () => {
        ...
        const s = await session.page.evaluate((payload) => window.tooboooring.renderFrame(payload), {
          project,
          media,
          frame: body.frame,
    ...
    res.writeHead(200, {
      ...
      'X-TooBoooringStudio-Frame': String(summary.frame),
      ...(missing.length ? { 'X-TooBoooringStudio-Missing-Media': asciiHeader(missing) } : {}),
    })
  ...
  const handleLayout = async (req, res) => {
    ...
    const layout = await queue.enqueue(
      () =>
        session.page.evaluate((payload) => window.tooboooring.dumpLayout(payload), {
          project,
          media,
          frame: body.frame,
          atSeconds: body.at ?? body.atSeconds,
  ...
  console.log(`TooBoooringStudio render service on http://${host}:${port}  (workspace: ${workspace})`)
  console.log(
