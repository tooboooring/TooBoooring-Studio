...
const HOST_SOURCE = 'tooboooring-moss-tts-worker';
const CLIENT_SOURCE = 'tooboooring-moss-tts-client';
const TOKENIZER_SOURCE = 'nano-reader-tokenizer-sandbox';
const MODEL_STORE_KEY = 'tooboooring-moss-tts';
...
self.__tooboooringMossTokenizerBridge = postTokenizerRequest;
...
async function getRuntime({ requestId, threadCount = 4, warmup = false }) {
  ...
  if (warmup && !runtime.__tooboooringWarmedUp) {
    postToMain({
      type: 'progress',
      requestId,
    ...
    runtime.__tooboooringWarmedUp = true;
  }
